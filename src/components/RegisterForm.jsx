    ...
    <>
      <form onSubmit={handleSubmit} className="mb-6 w-full lg:max-w-[65%] flex flex-col gap-4 border-2 p-4 rounded-md shadow-md">
        <div className="flex justify-center">
          ...
          />
          <div onClick={handleSelectImage} className="relative rounded-full w-[140px] h-[140px] border-2 overflow-hidden border-black/80  hover:border-black ease-out duration-300 hover:cursor-pointer hover:shadow-lg flex items-center justify-center">          
            <Image
              src={selectedImage ? selectedImage : "/images/selection.png"}
              width={140}
              height={140}
              className={`object-cover ${selectedImage ? 'w-full h-full' : 'w-[55%] md:w-[60%] h-[55%] md:h-[60%]'}`}
              alt="Img User"
            />             
          </div>        
        </div>
